...
#paired t-test
{
  ...
  #write.csv(paired_t_tests, "Spawn.decision.latency.paired.t.csv", row.names = TRUE)
...
## point plot
{
  ...
  point_plot <- ggplot(All_trans_grouped, aes(x =Mean_Transition_time ,y = interaction_order, color = Subject)) +
    ...
    scale_color_manual(values = c("Territorial" = "blue", "Non_Territorial" = "red"))
  
  # Print the point plot
  print(point_plot)
  #ggsave("Spawn_decidion_making_point_plot.png", plot = point_plot, width = 8, height = 6, units = "in", dpi = 300)
}
